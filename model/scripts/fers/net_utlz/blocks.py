...
from model.scripts.fers.net_utlz.utlz import initialize_weights_xavier
...
class UNetUp(nn.Module):
    def __init__(self, in_size, out_size, style_size=5):#6):
    ...
    def forward(self, x, skip_input, prob_t):
        ...
        if skip_input.shape[2:] != x.shape[2:]:
            skip_input = nn.functional.interpolate(skip_input, size=x.shape[2:], mode='bilinear', align_corners=False)
